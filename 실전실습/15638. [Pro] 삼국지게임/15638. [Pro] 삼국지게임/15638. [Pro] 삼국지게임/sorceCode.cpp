...
struct NationInfo{int x;int y;int soldier;};
NationInfo Nation[M_ATTACK + M_N * M_N];
...
void init(int N, int mSoldier[25][25], char mMonarch[25][25][11])
{
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			NationMap[mMonarch[i][j]] = index;

			NationIndex[i][j] = index;

			Nation[index].soldier = mSoldier[i][j];
			Nation[index].x = i;
			Nation[index].y = j;
			parent[index] = index;
...
int attack(char mMonarchA[11], char mMonarchB[11], char mGeneral[11])
{
	...
	for (int i = dx[0]; i <= dx[1]; i++) {
		for (int j = dy[0]; j <= dy[1]; j++) {
			if (bossA == Find(NationIndex[i][j])) {
				moveNum = Nation[NationIndex[i][j]].soldier / 2;
				Nation[NationIndex[i][j]].soldier -= moveNum;
				totalSolA += moveNum;
			}
			else if (bossB == Find(NationIndex[i][j])) {
				moveNum = Nation[NationIndex[i][j]].soldier / 2;
				Nation[NationIndex[i][j]].soldier -= moveNum;
				totalSolB += moveNum;
			}
	...
	NationIndex[Nation[NationCount].x][Nation[NationCount].y] = NationCount;
